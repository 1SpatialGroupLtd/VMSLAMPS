 ...
 *
 * Timestamp of this file for the 2002 release was: 1990-10-02 15:30:04.000000000 +0100
 *
...
int pkz_code, wan_code, wen_code, pnr_code, w_code;
...
/*----------------------------------------------------------------------*/
{
   ...
   /* workspace variables */
   int len, i, c, cnt;		/* workspace variables */
   int name_len;
   ...
   while (entry_code != -1)	/* retrieve entries sequentially */
   {
	 ...
	 if (v_info.flags & PEN_DOWN)
	 {
	    for ( i=0; i < v_info.npts; i++ )
	    {
	       ...
	       {
		  /* too many coordinates to fit buffer,
		   * output the old buffer
		   */
		  output();
		   ...
		   * last point of old block becomes first of new
		   */
		  if ( i < (v_info.npts-1) )
		  {
	    ...
	    /* new feature or invisible line break */
	 {
...
fprintf( (fout), "HGEL %14.4f\n", v_info.vertex[(j)].z )
 ...
 /*----------------------------------------------------------------------*/
	 ...
	 if (fc_data.poly)	/* create polylinea */
	 {
	    FPRINTF_STARS( fout );
